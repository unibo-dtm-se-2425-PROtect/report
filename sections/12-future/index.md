...
- In the CLI not all argparse commands work properly (ex. the flag "reconfig" does not properly point to the steps that should be done despite the code for such implementation has been coded, it stops halfway and mostly points to the "config" command instead, although I tried to specify the difference between the two;
...
- For future improvements, we could add some useful functionalities that now are missing (like enabling the transition of the button "lock" to "unlock" by reinserting the master password). 
- Reintroduce a dedicated tool in the GUI (and CLI) to generate cryptographically secure, random passwords for new entries
- The current cryptography uses AES-256 and a simple SHA256 hash derived from the Master Password (MP) for storage and key generation. This area needs improvement to protect against modern cracking techniques (replace hashlib with something like PBKDF2). 
- Integrate a real-time visual password strength indicator into the entry creation/editing forms.
