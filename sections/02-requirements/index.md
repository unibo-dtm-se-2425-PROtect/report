...
| F8  | The system must **validate input** so that the required fields (Site, URL, Username, Password) are not empty before saving. | checked when the user tries to save a new entry or a modified row and an error message pops up warning that some of the compulsory fields appear empty. |
...
| NF4 | The system must use **strong encryption** to secure passwords. (e.g., AES standard, sha-256 hashing function) | checked when all stored passwords are encrypted before saving and cannot be retrieved in plaintext without proper decryption. |
| NF5 | The system must **strengthen the master password** by hashing it with SHA-256 and comparing it to the stored hash in the database. | checked when the hash function works properly with the right length (e.g., 32 bytes for AES-256) and verifying the same master password with the same device secret produces the same key. |
...
| NF9 | The device secret must be generated using a secure **random secure mechanism** to guarantee uniqueness. | checked when strong, widely recognized encryption standard (such as SHA) are utilized. |
...
| I5  | When the master password is provided as ASCII, the system computes its **SHA-256 hash and uses this 32-byte digest** | checked when SHA-256 is explicitly used as the hash function, the derived key length matches the required AES key size, the derivation process correctly combines the master password and the device secret (salt) and using the same master password and device secret produces consistent keys on repeated derivations. |
